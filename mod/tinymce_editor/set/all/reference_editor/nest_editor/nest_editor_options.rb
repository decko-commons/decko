format :html do
  ...
  def nest_option_name_select selected=nil, level=0, include_view_option=true
  ...
  def nest_view_select selected
    select_tag :view, options_for_select(view_list.unshift("--"), selected: selected), class: "tags _view-select _nest-option-value"
  end
  ...
  def nest_size_select_template
    nest_option_value_select_tag :size, options_for_select(%w[small medium large original], selected: "medium")
  end
