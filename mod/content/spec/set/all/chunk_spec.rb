RSpec.describe Card::Set::All::Chunk do
  describe "#edit_fields" do
    ...
    it "doesn't make card objects to fields" do
      ...
      expect(format.render_edit).to have_tag ".card-editor", with: { "data-card-name": "B" }
    end

    it "treats symbols as codenames", as_bot: true do
      format = format_with_edit_fields %i[write basic]
      expect(format.render_edit).to have_tag ".card-slot" do
        with_tag ".card-editor.RIGHT-Xwrite", with: { "data-card-name": "A+*write" }
        with_tag ".card-editor.RIGHT-rich_text", with: { "data-card-name": "A+RichText" }
      end
    end
    ...
    example "absolute option", as_bot: true do
      ...
      expect(format.render_edit).to have_tag ".card-editor", with: { "data-card-name": "*self" }
    end
