RSpec.describe Card::Set::All::Admin do
  describe "all_admin_configs_of_category" do
    it "finds settings" do
      expect(Card[:all].all_admin_configs_of_category("settings").map { |x| x.codename })
          .to include("create")
    end
  ...
  specify "admin_config_by_role" do
    ...
    expect(roles[:anyone_signed_in].map { |c| c.codename})
      .to contain_exactly("mod", "user", "setting", "json", "number", "plain_text", "toggle", "phrase", "uri",
                          ...
                          "date", "notification_template", "session", "basic", "search_type", "signup")
  end
end
